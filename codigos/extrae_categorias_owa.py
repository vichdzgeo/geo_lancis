...
def matriz_transicion(dicc_capas,categorias):
...
#    encabezado = "categoria,pixeles\n"
#    
#    matrix_csv.write(encabezado)
#    for k1,v1 in matrix.items():
#       renglon = [str(k1),str(v1[k1-1])]
...
def categorias_pixeles(path_capa):
    dicc_capas = {'capa_1':{'ruta':path_capa},
                  'capa_2':{'ruta':path_capa}
                }

    categorias = int(raster_max(dicc_capas['capa_1']['ruta']))
    matriz = matriz_transicion(dicc_capas,categorias)
    return matriz 
    

path_directorio = 'C:/Dropbox (LANCIS)/SIG/desarrollo/sig_papiit/procesamiento/vulnerabilidad_costera/clasificaciones/'
capas=[]
for root, dirs, files in os.walk(path_directorio):
    capas+=([root+f for f in os.listdir(root) if isfile(join(root, f))if f.split(".")[-1:][0]=='tif'])

nombres_principales = []
for capa in capas:
    nombre_principal ="_".join(nombre_capa(capa).split("_")[0:3])
    nombres_principales.append(nombre_principal)

nombres_principales =list(set(nombres_principales))

for nombre_p  in nombres_principales:
    criterio = nombre_p.split("_")[0]
    
    capas_separadas =[]
    path_csv ='C:/Dropbox (LANCIS)/SIG/desarrollo/sig_papiit/procesamiento/vulnerabilidad_costera/'+criterio+".csv"
    matrix_csv = open(path_csv,"a")
    encabezado = "criterio,clasificador,fp,categoria,pixeles\n"
    matrix_csv.write(encabezado)
    for capa in capas: 
        if nombre_p in capa:
            capas_separadas.append(capa)
    for capa_separada in capas_separadas:
        if "equidistante" in capa_separada:
            nombre_clasificador = "equidistante"
            fp=1
        else:
            clasificador = nombre_capa(capa_separada).split("_")[3:6]
            nombre_clasificador = clasificador[0]
            fp = ".".join(clasificador[1:3])
        
        matriz = categorias_pixeles(capa_separada)
        
        for k1,v1 in matriz.items():
            renglon = [criterio,nombre_clasificador,str(fp),str(k1),str(v1[k1-1])]
            matrix_csv.write(",".join(renglon)+"\n")
