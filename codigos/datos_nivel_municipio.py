...
for clasificacion in range(2,5):
    ...
    for i in range(1,7):
        ...
        serie = 'mun_usv_s'+str(i)+"_c"+str(clasificacion)
        #Ruta de la capa vectorial que tendrá la información a nivel municipal
        path_mun_usv= 'C:/Dropbox (LANCIS)/CARPETAS_TRABAJO/vhernandez/geo_lancis/nivel_municipios/clasificacion_'+str(clasificacion)+"/"+serie+'.shp'
        path_mun_usv_csv= 'C:/Dropbox (LANCIS)/CARPETAS_TRABAJO/vhernandez/geo_lancis/nivel_municipios/clasificacion_'+str(clasificacion)+"/"+serie+'.csv'
