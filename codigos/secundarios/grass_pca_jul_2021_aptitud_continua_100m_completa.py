...
def getPuntoDeCorte(data, numCategorias):
    ...
    return corte
def getPuntoDeCorte2(data): ###lOIZER NO HACER CASP DE ESTA FUNCIÓN
...
def separar_pca(capa,corte,path_salida):
    ...
    for salida in capas_salidas:
        ...
        gscript.run_command('r.out.gdal',flags='cm',
                    ...
                    output=path_salida+salida.replace("mask_","grupo_").replace('pca1',"").replace(".1","")+'.tif',
...
def main():
    ...
    ruta = 'C:/Dropbox (LANCIS)/SIG/desarrollo/sig_fomix/procesamiento/analisis_aptitud/politetico_divisivo/grass/grupos_100m/'
    capas = [
    ...
    corte =getPuntoDeCorte(data)
    print(nombre_pca,"corte en ",corte)
    mascaras = separar_pca(capa_pca,corte,ruta)#6
    ...
    corte =getPuntoDeCorte(data)
    print(nombre_pca,"corte en ",corte)
    mascaras = separar_pca(capa_pca,corte,ruta) #4
    ...
    if len(data)>0:
        corte =getPuntoDeCorte(data)
        print(nombre_pca,"corte en ",corte)
        mascaras = separar_pca(capa_pca,corte,ruta)
        ...
        if len(data)>0:
            ...
            #corte = getPuntoDeCorte(data,11)
            #print(nombre_pca,"corte en ",corte)
            # mascaras = separar_pca(capa_pca,corte,ruta) #7
    ...
    corte =getPuntoDeCorte(data) # regresa el intervalo que sera el corte
    print(nombre_pca,"corte en ",corte)
    mascaras = separar_pca(capa_pca,corte,ruta)#5   ## separa el pca dado el numero de corte
    ...
    # p_csv = ruta+'promedios_grupos.csv'
    # name_columns = ['grupo']

    # for capa in capas:
    #    name_columns.append(capa.replace("@PERMANENT",""))

    # df = pd.read_csv(p_csv ,names=name_columns)
    # df_promedios =df.groupby('grupo').mean()
    # df_promedios.to_csv(ruta+"bd_promedios_grupos.csv",index = True, header=True)
